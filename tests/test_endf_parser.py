...
def test_endf_parserpy_never_fails(endf_file, myEndfParser, mf_sel):
    try:
        endfdic = myEndfParser.parsefile(endf_file, include=mf_sel)
...
def test_endf_read_write_read_roundtrip_preserves_content(
    endf_file, tmp_path, myEndfParser, mf_sel
):
    endf_dic = myEndfParser.parsefile(endf_file, include=mf_sel)
    outfile = tmp_path / os.path.basename(endf_file)
    myEndfParser.writefile(outfile, endf_dic)
    endf_dic2 = myEndfParser.parsefile(outfile, include=mf_sel)
    ...
    raw_endf = myEndfParser.parsefile(endf_file, include=tuple())
    raw_endf2 = myEndfParser.parsefile(outfile, include=tuple())
